...
from core.intelligence_synthesis import TieredGenerator, synthesize_period_summary
...
from workflows.period_intelligence import PeriodIntelligenceReport, build_monthly_report_from_memory


@dataclass(frozen=True)
class PeriodPipelineResult:
    report: PeriodIntelligenceReport
    brief: BriefRecord
...
def run_monthly_pipeline(
    *,
    ...
    report = build_monthly_report_from_memory(store, period_start=period_start, period_end=period_end)
    return _run_period_delivery(
        store=store,
        report=report,
...
def _run_period_delivery(
    *,
    ...
    model_router: TieredGenerator | None,
    publish_notion: bool,
    send_telegram: bool,
) -> PeriodPipelineResult:
    report = type(report)(
        brief_type=report.brief_type,
        title=report.title,
        executive_summary=synthesize_period_summary(
            title=report.title,
...
def _brief_record(report: PeriodIntelligenceReport, period_end: str) -> NotionBriefRecord:
    ...
    return NotionBriefRecord(
        ...
        intelligence_score=_period_score(report),
        confidence="medium" if report.top_actions else "low",
        status="Published",
        tags=("AI Intelligence", f"{report.brief_type.title()} Report"),
        body="\n".join(body_lines),
    )


def _notification(report: PeriodIntelligenceReport, notion_url: str) -> TelegramNotification:
...
def _period_score(report: PeriodIntelligenceReport) -> int:
